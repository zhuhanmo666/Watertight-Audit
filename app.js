...
async function loadSummary() {
  ...
  ui.reason.replaceChildren(new Option('全部未交付原因', ''));
...
function renderModelList() {
  ...
  state.models.forEach((item, index) => {
    ...
    meta.textContent = item.category === 'fail'
      ? `${(item.failReasons || []).join(', ') || '未交付'} · ${formatSeconds(item.processingSeconds)}`
      : `${partText ?? '?'} parts · ${formatBytes(item.bytes)} · ${formatSeconds(item.processingSeconds)}`;
...
function categoryLabel(category) {
  return category === 'pass' ? 'SUCCESS' : 'UNDELIVERED';
}
...
function renderDetails(item) {
  ...
  ui.meshCount.textContent = item.category === 'pass' ? '载入中' : '—';
  ...
  if (item.category === 'pass') {
    ...
    const assemblyTag = document.createElement('span');
    assemblyTag.className = item.assemblyFidelity === 'PASS'
      ? 'reason-tag pass-tag'
      : 'reason-tag neutral-tag';
    assemblyTag.textContent = item.assemblyFidelity === 'PASS'
      ? '装配保真：通过'
      : '装配保真：未记录';
    ui.failReasons.append(assemblyTag);
  } else {
    (item.failReasons || ['未交付']).forEach((reason) => {
      const tag = document.createElement('span');
